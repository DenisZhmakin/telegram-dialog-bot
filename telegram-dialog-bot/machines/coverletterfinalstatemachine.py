...
from keyboard import start_keyboard, main_keyboard, print_keyboard, create_arrows_keyboard
from machines.abstractfinalstatemachine import AbstractFinalStateMachine


class CoverLetterFinalStateMachine(AbstractFinalStateMachine):
    def __init__(self, bot: TeleBot, parent_trigger: Callable):
        self.data = loads(Path("data/cover-letter.json").read_text())
    ...
    def initialize(self, message: Message):
        ...
        self.bot.reply_to(message, "Добро пожаловать в режим `Формирование сопроводительного письма`.",
    ...
    def print_document(self, message: Message):
        document = DocxTemplate("files/cover-letter.docx")
        context = {}
        ...
        file = BytesIO()
        file.name = 'Сопроводительное письмо.docx'
        document.save(file)
        file.seek(0)
