...
class ConfirmationLetterFinalStateMachine(AbstractFinalStateMachine):
    def __init__(self, bot: TeleBot, parent_trigger: Callable):
        self.data = loads(Path("data/confirmation-letter.json").read_text())
    ...
    def print_document(self, message: Message):
        document = DocxTemplate("files/confirmation-letter.docx")
        context = {}
        ...
        file = BytesIO()
        file.name = 'Письмо подтверждение.docx'
        document.save(file)
        file.seek(0)
